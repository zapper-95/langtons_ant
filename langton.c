...
// Turns the ant left depending on its current direction
...
// Turns the ant right depending on its current direction
...
void move_forward(struct ant *ant) {
    // Moves the ant up because y=max_y-1 is at the lowest point
    // and this decreases as you go upwards
    if (ant->direction == UP) {
        ant->y += -1;
    } else if (ant->direction == LEFT) {
        ant->x += -1;
    } else if (ant->direction == DOWN) {
        ant->y += 1;   // Moves the ant down because y=0 is at the highest point
                       // and this increases as you go downwards
    } else {
        ant->x += 1;
    }
}
...
void apply_rule_general(int *colour, struct ant *ant, struct rule *rule) {
    ...
    // we do this by checking the rules string at the index
    ...
    if ((rule->rules)[state] == 'L') {
        turn_left(ant);
    } else {  // if the rule is R
        turn_right(ant);
    }
    // increment the current state by 1
    // When we get to the end of the rules string we wrap around to the start
