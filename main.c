...
int main(int argc, char* argv[]){
		...
		if(argc > 1){ //if the user has entered a rule, use that rule
			...
			current_rule = argv[1]; //the rule is assumed to be contained in only the first argument
...
//check the rule is valid by iterating through each character and checking it is either an R or L
bool ValidRule(char* rule){
	int i = 0;
	while(rule[i] != '\0'){
